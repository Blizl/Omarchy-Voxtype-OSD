...
simulate_omarchy_theme_set() {
  ...
  if [[ ! -f "$src_colors" ]]; then
    fail "Official theme colors file not found: $src_colors"
  fi
...
for entry in "${THEMES[@]}"; do
  ...
  simulate_omarchy_theme_set "$theme_name"
...
if command -v qs >/dev/null 2>&1; then
  # Initialize with nord
  simulate_omarchy_theme_set "nord"

  HARNESS_QML="$WORK_DIR/ThemeHarness.qml"
  cat << 'EOF' > "$HARNESS_QML"
import QtQuick
import Quickshell
...
ShellRoot {
    ...
    Connections {
        ...
        function onThemeModeChanged() {
            console.log("MODE_CHANGED:" + VT.Theme.currentThemeName + ":" + VT.Theme.themeMode);
        }
    }
...
EOF
  ...
  # Copy voxtype-shared into work dir so harness can load it
  cp -r "$ROOT/voxtype-shared" "$WORK_DIR/voxtype-shared"
